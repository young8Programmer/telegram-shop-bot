      ...
      try {
        ...
        if (data === 'add_category') {
          ...
          await this.telegramService.sendMessage(chatId, message, { 
            reply_markup: { 
              force_reply: true,
              inline_keyboard: getAdminKeyboard(language)
            } 
          });
          ...
          const message = categories.length ? formatCategoryList(categories, language) : 
            language === 'uz' ? '❌ Kategoriyalar mavjud emas.' : '❌ Категории отсутствуют.';
          await this.telegramService.sendMessage(chatId, message, {
            parse_mode: 'HTML',
            reply_markup: { inline_keyboard: getAdminKeyboard(language) },
          });
          ...
          if (!categories.length) {
            const message = language === 'uz' ? '❌ Kategoriyalar mavjud emas.' : '❌ Категории отсутствуют.';
            await this.telegramService.sendMessage(chatId, message, {
              reply_markup: { inline_keyboard: getAdminKeyboard(language) },
            });
            return;
          ...
          const message = products.length ? formatProductList(products, language) : 
            language === 'uz' ? '❌ Mahsulotlar mavjud emas.' : '❌ Товары отсутствуют.';
          await this.telegramService.sendMessage(chatId, message, {
            parse_mode: 'HTML',
            reply_markup: { inline_keyboard: getAdminKeyboard(language) },
          });
          ...
          const products = await this.productService.findAll();
          if (!products.length) {
            const message = language === 'uz' ? '❌ Mahsulotlar mavjud emas.' : '❌ Товары отсутствуют.';
            await this.telegramService.sendMessage(chatId, message, {
              reply_markup: { inline_keyboard: getAdminKeyboard(language) },
            });
            return;
          ...
          const message = users.length ? formatUserList(users, language) : 
            language === 'uz' ? '❌ Foydalanuvchilar mavjud emas.' : '❌ Пользователи отсутствуют.';
          await this.telegramService.sendMessage(chatId, message, {
            parse_mode: 'HTML',
            reply_markup: { inline_keyboard: getAdminKeyboard(language) },
          });
          ...
          const users = await this.userService.findAll();
          if (!users.length) {
            const message = language === 'uz' ? '❌ Foydalanuvchilar mavjud emas.' : '❌ Пользователи отсутствуют.';
            await this.telegramService.sendMessage(chatId, message, {
              reply_markup: { inline_keyboard: getAdminKeyboard(language) },
            });
            return;
          ...
          await this.telegramService.sendMessage(chatId, message, {
            reply_markup: { inline_keyboard: keyboard.length ? keyboard : getAdminKeyboard(language) },
            parse_mode: 'HTML',
          });
          ...
          const message = deliveries.length ? formatDeliveryList(deliveries, language) : 
            language === 'uz' ? '❌ Yetkazib berishlar mavjud emas.' : '❌ Доставки отсутствуют.';
          ...
          if (!deliveries.length) {
            const message = language === 'uz' ? '❌ Yetkazib berishlar mavjud emas.' : '❌ Доставки отсутствуют.';
            await this.telegramService.sendMessage(chatId, message, {
              reply_markup: { inline_keyboard: getAdminKeyboard(language) },
            });
            return;
          ...
          if (!delivery) {
            const message = language === 'uz' 
              ? `❌ Yetkazib berish ID ${deliveryId} topilmadi.` 
          ...
          const message = feedbacks.length ? formatFeedbackList(feedbacks, language) : 
            language === 'uz' ? '❌ Feedbacklar mavjud emas.' : '❌ Отзывы отсутствуют.';
          await this.telegramService.sendMessage(chatId, message, {
            parse_mode: 'HTML',
            reply_markup: { inline_keyboard: getAdminKeyboard(language) },
          });
          ...
          const feedbacks = await this.feedbackService.findAll();
          if (!feedbacks.length) {
            const message = language === 'uz' ? '❌ Feedbacklar mavjud emas.' : '❌ Отзывы отсутствуют.';
            await this.telegramService.sendMessage(chatId, message, {
              reply_markup: { inline_keyboard: getAdminKeyboard(language) },
            });
            return;
          ...
          if (!feedback) {
            const message = language === 'uz' 
              ? `❌ Feedback ID ${feedbackId} topilmadi.` 
              : `❌ Отзыв с ID ${feedbackId} не найден.`;
            await this.telegramService.sendMessage(chatId, message, {
              reply_markup: { inline_keyboard: getAdminKeyboard(language) },
            });
            return;
          ...
          const message = stats ? formatStats(stats, language) : 
            language === 'uz' ? '❌ Statistikalar mavjud emas.' : '❌ Статистика отсутствует.';
          await this.telegramService.sendMessage(chatId, message, {
            parse_mode: 'HTML',
            reply_markup: { inline_keyboard: getAdminKeyboard(language) },
          });
